...
| 명령어 | 주요 용도 | 실행 예시 |
| :--- | :--- | :--- |
| **`ai`** | **완전 자율형 에이전트 (2026년 구글 검색 + PC 제어 + 대화 기억)** | `ai "내 그래픽카드 온도 확인하고 최신 뉴스 검색해줘"` |
| **`ai clear`** | **대화 세션 영구 기억 즉시 초기화** | `ai clear` 또는 `ai reset` |
| **`ask`** | 고속 단발 질의 및 파일 파이프라인 분석 (`smart-qwen` 우선) | `cat config.conf \| ask "설정 오류 검토해줘"` |
| **`askweb`** | Google 한국어 실시간 RAG 검색 (2026년 기준 쿼리 보정) | `askweb "오늘 환율 및 주가 알려줘"` |
